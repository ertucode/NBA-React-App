...
export default function TextInputField({ zeroPlayer, setDesiredPlayers }) {
  ...
  useEffect(() => {
    inputRef.current.parentNode.classList.toggle("loading", inputsAreDifferent);
  }, [inputsAreDifferent]);

  useEffect(() => {
    if (!debouncedPlayerSearchTerm) return;
    ...
    setDesiredPlayers((prevDesiredPlayers) => {
      ...
      if (
        prevDesiredPlayers.find(
          (prevDesiredPlayer) => prevDesiredPlayer.id === newDesiredPlayer.id,
        )
      ) {
    ...
    <>
      <div className={`input-field ${zeroPlayer ? "no-player" : ""}`}>
        <div className="input-with-dropdown">
          <input
            className={
              searchedPlayers.length === 0 || !showDropdown ? "empty" : ""
            }
            onChange={(e) => {
